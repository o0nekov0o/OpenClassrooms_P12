...
class ContractViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        customer, commercial_contact, total_amount, unpaid_amount, creation_date, contract_state = \
            changed_contract['customer'], changed_contract['commercial_contact'], \
            changed_contract['total_amount'], changed_contract['unpaid_amount'], \
            changed_contract['creation_date'], changed_contract['contract_state']
        if customer == '' or commercial_contact == '' or total_amount == '' or unpaid_amount == '' \
                or creation_date == '' or contract_state == '':
            return response.Response(status=status.HTTP_401_UNAUTHORIZED)
        if request.user.is_superuser or request.user.collaborator_type == 0 \
    ...
    def create(self, request, *args, **kwargs):
        ...
        customer, commercial_contact, total_amount, unpaid_amount, creation_date, contract_state = \
            new_contract['customer'], new_contract['commercial_contact'], new_contract['total_amount'], \
            new_contract['unpaid_amount'], new_contract['creation_date'], new_contract['contract_state']
        if customer == '' or commercial_contact == '' or total_amount == '' or unpaid_amount == '' \
                or creation_date == '' or contract_state == '':
            return response.Response(status=status.HTTP_401_UNAUTHORIZED)
        if request.user.is_superuser or request.user.collaborator_type == 0:
            models.Contract.objects.create(customer=customer, commercial_contact=commercial_contact,
                                           total_amount=total_amount, unpaid_amount=unpaid_amount,
                                           creation_date=creation_date, contract_state=contract_state)
            return response.Response(status=status.HTTP_201_CREATED)
        else:
            return response.Response(status=status.HTTP_401_UNAUTHORIZED)
...
class EventsViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        contract, customer_name, customer_contact, event_start_date, \
        event_end_date, support_contact, location, attendees, notes = \
            changed_event['contract'], changed_event['customer_name'], \
            changed_event['customer_contact'], changed_event['event_start_date'], \
        ...
        if contract == '' or customer_name == '' or customer_contact == '' \
                or event_start_date == '' or event_end_date == '' or support_contact == '' \
                or location == '' or attendees == '' or notes == '':
            return response.Response(status=status.HTTP_401_UNAUTHORIZED)
        if request.user.is_superuser or request.user.collaborator_type == 0 \
                or instance.contract.customer.creator == request.user \
    ...
    def create(self, request, *args, **kwargs):
        ...
        contract, customer_name, customer_contact, event_start_date, \
        event_end_date, support_contact, location, attendees, notes = \
            new_event['contract'], new_event['customer_name'], \
            new_event['customer_contact'], new_event['event_start_date'], \
        ...
        if request.user.is_superuser or request.user.collaborator_type == 0 \
            ...
            models.Event.objects.create(contract=contract, customer_name=customer_name,
                                        customer_contact=customer_contact, event_start_date=event_start_date,
